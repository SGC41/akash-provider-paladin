# Akash Provider Paladin v2.12.0
...
- v2.12.0  - Improved functionality with single control plane providers.
...
- v2.11.13 - Changed Check Provider Bids script, so that it won't constantly bounce the Provider Pod is there are low number of bids.
             Added custom variable for the required Provider Pod age for a bounce... found in scripts/default.conf

- v2.11.12 -Fixed multiple scheduled executions.
            added flock lock to ticker-control-plane.
            
            fixed cron line install again, again..... ended up reverting to old code.

            added display script - will get more functions in the future.
...
- v2.8.0
     Changed ticker.sh to reduce provider pod restart detection delay, should hopefully be immediately now, rather than delayed.
