...
SCRIPT_PATH="$TARGET_DIR/scripts/ticker-control-plane.sh"
...
root@cnode1:~/akash-provider-paladin# git checkout unstable
Switched to branch 'unstable'
Your branch is ahead of 'origin/unstable' by 1 commit.
  (use "git push" to publish your local commits)
root@cnode1:~/akash-provider-paladin# nano install.sh
root@cnode1:~/akash-provider-paladin# git checkout stable
Switched to branch 'stable'
Your branch is behind 'origin/stable' by 25 commits, and can be fast-forwarded.
  (use "git pull" to update your local branch)
root@cnode1:~/akash-provider-paladin# nano install.sh
root@cnode1:~/akash-provider-paladin# cat install.sh
#!/usr/bin/env bash
set -euo pipefail
