...
#Minimum price module start

TARGET_MINIMUM_PRICE="10"  # USD/Provider minimum price a month

#Minimum Accepted Lease Cost Check
#total_cost_usd_target
      if ! (( $(echo "$total_cost_usd_target > 0" | bc -l) && \
              $(echo "$total_cost_usd_target >= $TARGET_MINIMUM_PRICE" | bc -l) )); then
        echo -n "$AKASH_OWNER requested deployment offer of $total_cost_usd_target which less than minimum accepted of $TARGET_MINIMUM_PRICE. Aborting!" >&2
        exit 1
      fi
#Minimum price module end
